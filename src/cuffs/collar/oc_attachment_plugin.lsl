...
//    TODO: consolidate these so the message to the attached device can be done all in one, not one at a time

HandleBulkResponse(string sResponse) {
    string sCuffsMessage = "";
    string jsonvalue = llJsonGetValue(sResponse, ["cuffs_chaintex"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_kTexture = jsonvalue;
        sCuffsMessage = llJsonSetValue(sCuffsMessage, ["cuffs_chaintex"], g_kTexture);
    }
//    HandleSettings("cuffs_chaintex=" + jsonvalue);
    jsonvalue = llJsonGetValue(sResponse, ["cuffs_synclock"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_bSyncLock = IsTrue(jsonvalue);
        sCuffsMessage = llJsonSetValue(sCuffsMessage, ["cuffs_synclock"], jsonvalue);
    }
//    HandleSettings("cuffs_synclock=" + jsonvalue);
    jsonvalue = llJsonGetValue(sResponse, ["global_locked"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_bCollarLocked = IsTrue(jsonvalue);
        sCuffsMessage = llJsonSetValue(sCuffsMessage, ["global_locked"], jsonvalue);
    }
//    HandleSettings("global_locked=" + jsonvalue);
    jsonvalue = llJsonGetValue(sResponse, ["cuffs_poses"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_lActivePoses = llParseString2List(jsonvalue,[","],[]);
        sCuffsMessage = llJsonSetValue(sCuffsMessage, ["cuffs_poses"], jsonvalue);
    }
//    HandleSettings("cuffs_poses=" + jsonvalue);
    jsonvalue = llJsonGetValue(sResponse, ["cuffs_lock"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_bLocked = IsTrue(jsonvalue);
        sCuffsMessage = llJsonSetValue(sCuffsMessage, ["cuffs_lock"], jsonvalue);
    }
//    HandleSettings("cuffs_lock=" + jsonvalue);
    jsonvalue = llJsonGetValue(sResponse, ["cuffs_hide"]);
    if (jsonvalue != JSON_NULL && jsonvalue != JSON_INVALID) {
        g_bHidden = IsTrue(jsonvalue);
         sCuffsMessage = llJsonSetValue(sCuffsMessage, ["cuffs_hide"], jsonvalue);
    }
//    HandleSettings("cuffs_hide=" + jsonvalue);
    if (sCuffsMessage != "")
        llRegionSayTo(g_kWearer, g_iChan_ocCmd, (string)g_kWearer+":JSON:"+sCuffsMessage);
}
...
*/
    ...
    timer() {
        ...
        BulkRequest("oc_attachment_plugin timer");
    }
}
