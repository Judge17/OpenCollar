...
//        if (kID!=g_kWearer && iNum!=CMD_OWNER) llMessageLinked(LINK_SET,NOTIFY,"0"+"%NOACCESS% to uninstall bookmarks",kID);
        ...
        if (iNum!=CMD_OWNER) llMessageLinked(LINK_SET,NOTIFY,"0"+"%NOACCESS% to add bookmarks",kID);
        else {
...
2) A new location or SLurl", [], [], 0, iNum,"TextBoxIdSave");
        ...
        if (iNum!=CMD_OWNER) llMessageLinked(LINK_SET,NOTIFY,"0"+"%NOACCESS% to remove bookmarks",kID);
        else {
